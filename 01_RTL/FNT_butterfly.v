/********************************************************************
* Filename: FNT_butterfly.v
* Authors:
*   Yin-Liang Chen
* Description:
*   Standalone 32-point forward FNT engine using butterfly method.
*
* Notes:
*   - Uses existing `RegFile` and `Dim1_BFU`.
*   - Forward FNT only (DIF schedule), 5 stages x 16 butterflies = 80 cycles.
...
module FNT_butterfly #(
    ...
    // ------------------------------------------------------------
    reg running_r;
    reg [6:0] counter_r; // 0..79 for 80 butterflies

    // fnt_stage[2:0], fnt_bfly[3:0] from counter
    wire [2:0] fnt_stage;
    wire [3:0] fnt_bfly;
    ...
    // ------------------------------------------------------------
    // Butterfly address + twiddle generator (same style as FrFT_core)
    // ------------------------------------------------------------
    always @(*) begin
        idxA = 5'd0;
        idxB = 5'd0;
    ...
    always @(*) begin
        ...
        if (running_r) begin
            ...
            bfu_shift       = {1'b0, twiddle_k};

            rf_write_addr_1 = idxA;
            rf_write_addr_2 = idxB;
