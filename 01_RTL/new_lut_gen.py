...
SCALE = 128           # 8-bit quantization scale
    ...
    with open(FILENAME, "w") as f:
        ...
        for k in range(1, 129):
            ...
            for idx in range(N//2): # idx 0~15
                val = c13_cr[idx]
                addr = (k-1) * 16 + idx
                f.write(f"            {int(np.log2(N))-1+KEY_WIDTH_13}'d{addr:<5}: out = {DATA_WIDTH}'b{val:018b};\n")
        ...
        # Traverse from k = 1 to 64 (Store Positive Key)
        for k in range(1, 65):
            a = k / 128.0
            alpha = a * math.pi # alpha = angle
            ...
            for idx in range(N//2): # idx 0~15
                val = c2_cr[idx]
                addr = (k-1) * 16 + idx
                f.write(f"            {int(np.log2(N))-1+KEY_WIDTH_2}'d{addr:<5}: out = {DATA_WIDTH}'b{val:018b};\n")
