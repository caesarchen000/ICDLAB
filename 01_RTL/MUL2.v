...
*   Yin-Liang Chen
* Description:
*   MUL2 stage for 2-path flow (integer-only arithmetic).
*
* Equation:
*   path1: P1 * (Cs + j*Cs_hat)
*   path2: P2 * (Cs - j*Cs_hat)
*
* Input format:
*   in_path*_data[32:0] = {path_flag, imag[15:0], real[15:0]}
*   (path flag is carried through, arithmetic uses real/imag only)
*
* Output format:
...
module MUL2 #(
    ...
    // ------------------------------------------------------------
    wire signed [31:0] o1_re_full = $signed(p1_re) * $signed(cs1_re) - $signed(p1_im) * $signed(cs1_im);
    wire signed [31:0] o1_im_full = $signed(p1_re) * $signed(cs1_im) + $signed(p1_im) * $signed(cs1_re);

    wire signed [31:0] o2_re_full = $signed(p2_re) * $signed(cs2_re) - $signed(p2_im) * $signed(cs2_im);
    wire signed [31:0] o2_im_full = $signed(p2_re) * $signed(cs2_im) + $signed(p2_im) * $signed(cs2_re);
