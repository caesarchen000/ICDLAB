...
module Dim1_Modular_Mul #(
    ...
    // 1. 解碼回真實數值 (0 ~ 2^32)
    wire [33:0] true_A = A[32] ? 34'd0 : {2'b0, A[31:0]} + 34'd1;
    wire [33:0] true_B = B[32] ? 34'd0 : {2'b0, B[31:0]} + 34'd1;

    // 2. 執行乘法 (最大值為 2^32 * 2^32 = 2^64)
    wire [67:0] prod = true_A * true_B;

    // 3. Fermat 摺疊: P_L - P_H (mod 2^32+1)
    wire [33:0] P_H = prod[67:32];
    wire [33:0] P_L = {2'b0, prod[31:0]};
